...
存档列表每行显示：名称、状态、**最近游玩时间**、大小与 level_id。时间来自
`level.dat` 的 `LastPlayed`；对于没有世界数据的条目（如在线存档）则退回到记录中
账号时间戳的最大值，详情页会标注该值并不精确。账号信息不在此列，见详情页。
...
写入的账号可在存档详情页的「账号」一节查看：那里列出账号数、每个账号及其登记
时间，以及本机是否已有对应目录。注意记录中的时间是账号的**登记时间**，不是游玩
时间；世界的游玩时间在「基本信息」的「最近游玩」一行。
